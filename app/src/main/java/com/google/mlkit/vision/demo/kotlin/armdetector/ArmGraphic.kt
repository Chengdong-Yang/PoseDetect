...
class ArmGraphic internal constructor(
    ...
    //private var prevElbowAngle = 0.0  // 上一帧的胳膊肘角度
    //private var wavingCount = 0  // 挥手次数
    override fun draw(canvas: Canvas) {
        ...
        while (abs(rightElbow!!.position.y - rightShoulder!!.position.y) < 0.05 && abs(rightWrist!!.position.y - rightShoulder!!.position.y) < 0.05) {
            break;
        }

        if (rightElbowAngle < 90.0 && (prevElbowAngle - rightElbowAngle) > 0.0) {
            if (isDown == true) {
                isDown = false
                isUp = true
                Count++
            }
        } else if (rightElbowAngle > 90.0 && (prevElbowAngle - rightElbowAngle) < 0.0) {
            if (isUp == true) {
                isUp = false
                isDown = true
                Count++
        ...
        // 更新上一帧的角度
        prevElbowAngle = rightElbowAngle

        drawText(canvas, lineOneText, 1)
        drawText(canvas, lineTwoText, 2)
        drawText(canvas, "count：" + Count, 3)
        /////////////////////
